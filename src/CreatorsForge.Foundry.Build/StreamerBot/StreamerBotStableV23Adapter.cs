...
public static class StreamerBotStableV23Adapter
{
        ...
        foreach (var action in definition.Actions)
        {
            ...
            for (var index = 0; index < action.SubActions.Count; index++)
            {
                ...
                subActions.Add(subAction.Kind switch
                {
                    ...
                    "executeCSharp" => CreateExecuteCSharpSubAction(
                        subAction, id, index, projectRoot),
                    _ => throw new InvalidOperationException(
    ...
    private static JsonObject CreateExecuteCSharpSubAction(
        ...
        string? projectRoot)
    {
        if (string.IsNullOrWhiteSpace(projectRoot))
            throw new InvalidOperationException("A project root is required to embed Execute C# source.");
        var path = StreamerBotCSharpAuthoringService.ResolveConfinedSourcePath(
            projectRoot, subAction.SourcePath!);
        var source = File.ReadAllText(path, new UTF8Encoding(false, true));
        if (Encoding.UTF8.GetByteCount(source) > StreamerBotCSharpAuthoringService.MaximumSourceBytes)
            throw new InvalidDataException($"Execute C# source '{subAction.SourcePath}' exceeds 1 MiB.");
        return new JsonObject
        {
            ["name"] = "Execute C# Code",
            ["description"] = subAction.Generation is null
                ? null
                : $"Generated by Foundry {subAction.Generation.Revision}; editable source",
            ["references"] = new JsonArray((subAction.References ?? [])
                .Select(value => (JsonNode?)JsonValue.Create(value)).ToArray()),
            ["byteCode"] = Convert.ToBase64String(new UTF8Encoding(false, true).GetBytes(source)),
            ["precompile"] = false,
            ["delayStart"] = false,
            ["saveResultToVariable"] = false,
            ["saveToVariable"] = null,
